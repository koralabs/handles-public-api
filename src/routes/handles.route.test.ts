...
describe('Testing Handles Routes', () => {
    ...
    describe('[POST] /handles/search', () => {
        it('should throw error if records_per_page is invalid', async () => {
            const response = await request(app?.getServer()).post('/handles/search').set('Content-Type', 'application/json').send({
                records_per_page: 'two'
            });
            expect(response.status).toEqual(400);
            expect(response.body.message).toEqual(ERROR_TEXT.HANDLE_LIMIT_INVALID_FORMAT);
        });
        ...
        it('should throw error if sort is invalid', async () => {
            const response = await request(app?.getServer()).post('/handles/search').set('Content-Type', 'application/json').send({ records_per_page: 1, sort: 'hmm' });
        ...
        it('should return handles', async () => {
            const response = await request(app?.getServer()).post('/handles/search').set('Content-Type', 'application/json').send({ records_per_page: 1, sort: 'asc' });
        ...
        it('should throw error if characters is invalid', async () => {
            const response = await request(app?.getServer()).post('/handles/search').set('Content-Type', 'application/json').send({ characters: 'nope' });
        ...
        it('should throw error if length is invalid', async () => {
            const response = await request(app?.getServer()).post('/handles/search').set('Content-Type', 'application/json').send({ length: 'nope' });
            expect(response.status).toEqual(400);
            expect(response.body.message).toEqual("Length must be a number or a range of numbers (ex: 1-28) and can't exceed 28");
        });

        it('should throw error if rarity is invalid', async () => {
            const response = await request(app?.getServer()).post('/handles/search').set('Content-Type', 'application/json').send({ rarity: 'nope' });
            expect(response.status).toEqual(400);
            expect(response.body.message).toEqual('rarity must be basic, common, rare, ultra_rare, legendary');
        });

        it('should throw error if numeric_modifiers is invalid', async () => {
            const response = await request(app?.getServer()).post('/handles/search').set('Content-Type', 'application/json').send({ numeric_modifiers: 'nope' });
        ...
        it('should throw error if page and slot number are used together', async () => {
            const response = await request(app?.getServer()).post('/handles/search').set('Content-Type', 'application/json').send({ page: 1, slot_number: 123 });
        ...
        it('should throw error if search query is less than 3 characters', async () => {
            const response = await request(app?.getServer()).post('/handles/search').set('Content-Type', 'application/json').send({ search: 'ab' });
        ...
        it('should pass plain text list of Accept is text/plain', async () => {
            const response = await request(app?.getServer()).post('/handles/search').set('api-key', 'valid-key').set('Accept', 'text/plain; charset=utf-8');
            expect(response.status).toEqual(200);
            expect(response.text).toEqual('burritos\ntacos\nbarbacoa');
        });
    });
